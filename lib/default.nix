np: input_machines: let
  ...
  machines = catLowerAttrs ("identity" input_machines);
in {
  lib = {
    inherit catLowerAttrs;
    inherit collectFlakeInputs;
    inherit exprsIn;
    inherit getAllOutputs;
    inherit machines;
    inherit mkEnableModule;
    inherit modulesIn;
    inherit versionAtMost;
  };
}
