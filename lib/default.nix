  ...
  lib = rec {
    ...
    modulesIn =
      ...
      np.pipe dir [
        ...
        (np.mapAttrsToList (
          name: type:
          if type == "regular" && np.hasSuffix ".nix" name && name != "default.nix" then
            [
              {
          ...
          else if type == "directory" && np.pathExists (dir + "/${name}/default.nix") then
            [
              {
                inherit name;
                value = dir + "/${name}";
      ...
      np.listToAttrs (
        np.concatMap (
          ...
          let
            v = set.${name};
          in
          if v ? ${key} then [ (np.nameValuePair name v.${key}) ] else [ ]
        ) (np.attrNames set)
      );

    # Collects the inputs of a flake recursively (with possible duplicates).
    collectFlakeInputs =
      input: [ input ] ++ np.concatMap np.collectFlakeInputs (builtins.attrValues (input.inputs or { }));

    # Gets all the outputs of a derivation as a list.
    getAllOutputs = drv: if drv ? outputs then np.attrVals drv.outputs drv else [ drv ];
    ...
    mkEnableModule = name: cfg: {
      ...
      imports = [
        (
          { config, ... }:
          {
            config = np.mkIf (np.getAttrFromPath name config).enable cfg;
          }
        )
      ];
    };
